...
//	UnicodeString newEntry = prefs.getRemoteIPEntry(0);
	...
	newIPAscii.translate(newIP);
	Int numFields = std::sscanf(newIPAscii.str(), "%d.%d.%d.%d", &(n1[0]), &(n1[1]), &(n1[2]), &(n1[3]));
	...
	for (Int i = 0; i < numEntries; ++i)
	{
		if (i != currentSelection)
		{
			...
			if (currentSelection == -1)
			{
				...
				AsciiString oldIPAscii;
				oldIPAscii.translate(oldIP);

				std::sscanf(oldIPAscii.str(), "%d.%d.%d.%d", &(n2[0]), &(n2[1]), &(n2[2]), &(n2[3]));
