...
def main():
    ...
    with TemporaryDirectory(prefix="zh-m22-terrain-water-") as scratch:
        ...
        os.environ["ZH_M22_TERRAIN_WATER_PROFILE"]="1"; os.environ["ZH_M22_TERRAIN_WATER_MAP"]=str(packet)
        ...
        if result.returncode or marker not in result.stdout: raise SystemExit(f"original terrain water failed ({result.returncode}); private output redacted")
