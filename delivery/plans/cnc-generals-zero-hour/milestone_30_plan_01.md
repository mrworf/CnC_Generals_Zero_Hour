...
Slice 04 implements SDL3 Wayland/X11 native-window handoff to a public bgfx swap-chain framebuffer and uses a public fullscreen textured draw to present the completed engine color target. Two device generations, release/reclaim, stale/null/double window negatives, an actual changed SDL3 pixel extent and swap-chain update pass on the RTX. Actual UI/world/post-effect shader pixels complement the slice-03 component pixels; this is renewed synthetic backend evidence, not a claim that M14's 16-scene SDL_GPU harness or M22's original retail scene ran unchanged. All four native presets build and their 174-test asset-free suites pass on the host. Sandbox UDP denial and third-party DBus/Wayland LSan allocations are separately graded in the evidence. An always-executed original strip-optimizer witness restores Release source-identity link-map coverage without relaxing the negative provider-removal test. M22 retains wiring the actual original WW3D call path into this device.
