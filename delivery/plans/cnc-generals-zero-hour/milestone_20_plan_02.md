...
| 02 | [Complete dispatch and live provider closure](milestone_20_plan_02_slice_02.md) | The complete production INI table and every referenced callback/provider body compile and link live; actual dispatch accepts a representative production block and rejects an unknown block without discarded-section or reduced-table evidence. | pending | — |
| 03 | [Complete data/configuration startup](milestone_20_plan_02_slice_03.md) | Original filesystem, INI/CSF/GlobalData/name-key, nonempty map-cache/localization, and GameState/Xfer/CRC consumers execute on owned fixtures with XDG-only writes. | pending | — |
