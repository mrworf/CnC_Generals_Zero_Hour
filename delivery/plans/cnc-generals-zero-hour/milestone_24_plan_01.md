...
| 05B | [slice 05B](milestone_24_plan_01_slice_05b.md) | Shipped GameData baseline survives reset; original first retail mission tick has bounded partition allocation | 05A | pending | | |
| 05C | [slice 05C](milestone_24_plan_01_slice_05c.md) | Retail skirmish exact source persistence through complete map/draw reconstruction | 05B + M22 §9 closure | pending — M22 provider blocked | | |
| 06 | [slice 06](milestone_24_plan_01_slice_06.md) | Retail save/replay/first-tick and four-preset cumulative M24 acceptance | 05C | pending | | |
...
The slice 05 retail skirmish probe subsequently reached the exact M22 physical draw-provider boundary during original save-load map reconstruction: `GameStateMap::xfer -> GameLogic::startNewGame(TRUE) -> ThingFactory::newDrawable -> ModuleFactory::newModule(MODULETYPE_DRAW)`, where `m_createProc` is null and the source throws `ERROR_INVALID_D3D`. Separately, the first original retail mission tick reached `ScriptActions::doBorderSwitch -> PartitionManager::init` with a 1-unit partition cell and attempted approximately 20.8 million cells: source reset discarded the shipped `PartitionCellSize = 40.0` override. The remainder is split into independently testable 05A mission/owned shroud and late-rollback repair, 05B shipped GameData baseline and first tick, 05C skirmish persistence after M22 §9 closure, and 06 cumulative acceptance. These are dependency corrections, not waived retail/CRC or first-tick requirements.
