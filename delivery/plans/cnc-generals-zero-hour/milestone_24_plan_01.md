...
| 05B | [slice 05B](milestone_24_plan_01_slice_05b.md) | Original GlobalData correctly owns copied default/shipped/map layers; shipped partition size survives reset | 05A | pending | | |
...
The slice 05 retail skirmish probe reached the M22 physical draw-provider boundary during original save-load map reconstruction: `GameStateMap::xfer -> GameLogic::startNewGame(TRUE) -> ThingFactory::newDrawable -> ModuleFactory::newModule(MODULETYPE_DRAW)`, where `m_createProc` is null and source throws `ERROR_INVALID_D3D`. Separately, the first retail mission tick previously reached `ScriptActions::doBorderSwitch -> PartitionManager::init` with a 1-unit cell and attempted approximately 20.8 million cells because reset discarded shipped `PartitionCellSize = 40.0`. Repairing the original unimplemented `GlobalData` copy and shipped ownership exposed an earlier authentic mission dependency: `GameLogic::startNewGame(FALSE):1868 -> ThingFactory::newDrawable -> Drawable -> ModuleFactory::newModule(MODULETYPE_DRAW)` for an optimized tree. The remaining 05B source baseline/lifecycle repair is independently testable; mission first-tick 05C and skirmish 05D require M22 §9, followed by cumulative 06. No draw, CRC or first-tick gate is waived.
