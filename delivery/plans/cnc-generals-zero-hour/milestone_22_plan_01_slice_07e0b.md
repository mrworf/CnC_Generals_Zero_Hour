...
In an initialized GameClient scenario, under an active original device edge and initialized W3DDisplay, publish the original W3DTerrainVisual as `TheTerrainVisual` before direct init, matching GameClient's factory-before-init order; the fixture restores its pre-existing headless visual only after original-owner teardown. The visual owns one terrain ref and the original track, shadow, water and smudge owners, attaches disabled water to the original 3D scene per 07E0B5, and keeps identities stable across idempotent init, empty reset and update. Reject no-edge/no-display, second visual, `load`, map-dependent or water/track/shadow operations without publishing a replacement or mutating a frame. Destruction detaches water and clears the child singletons before display teardown. Repeat over reset and independent device/display generations. Do not alter original class layout or native Windows branch; use typed rejection for unsupported virtual methods, not silent success.
