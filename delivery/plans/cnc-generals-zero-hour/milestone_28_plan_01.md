...
Slice 03 extracts the actual AudioSettings/MusicTrack/AudioEvent/DialogEvent field vocabulary and layered registry semantics, validates every logical resource before publication, and schedules owned music through the accepted native `AudioManager` in null-output mode. Missing music fails in under one second with its logical name and an explicit no-CD/no-modal diagnostic; no loaded state is fabricated. The checked dependency ledger now names the complete M20 lifecycle plus M21 map-start, M22 DX8 device lifecycle, M23 music flow, and M25 CRC-layout consumers.

Final validation passed the 10-test `original-resources` label in every GCC/Clang Debug/Release preset, the complete 92-test asset-free suite in every preset, and 10/10 focused Clang ASan/UBSan tests. Live-symbol identity, extraction provenance, source-drift/owner controls, provider removal, source classification, lifecycle counts, and no-device interception all pass. Acceptance details are recorded in the linked QA evidence.
