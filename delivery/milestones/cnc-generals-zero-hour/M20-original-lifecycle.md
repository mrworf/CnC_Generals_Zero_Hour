# Milestone M20: original engine initialization and data lifecycle

## Objective

Linux initializes original GameMain and GameEngine with real data and correct lifecycle ownership.
...
Implement Linux CreateGameEngine and original subsystem factories; adapt original FileSystem/LocalFileSystem/ArchiveFileSystem to BIG/VFS and XDG. Initialize original INI/CSF/object/module factories and globals; retain ordering and explicit null presentation services.

## Explicit Exclusions

Actual simulation and presentation remain later consumers. Required proprietary gaps follow the base-plan evidence gate. Preserve valid component implementations/tests. No ARM64, base Generals executable/tools, retail redistribution or gameplay rewrite.

## Source Requirements

[Migration supplement](../../../docs/zero-hour-source-engine-migration.md): SE-003; SE-010 assurance/evidence constraints. [Base port plan](../../../docs/zero-hour-linux-port-plan.md) §§1–6 and §10 govern preservation, stack, formats and validation. Revisions/commits are in status.yaml.

## Preconditions

Direct providers: M19, M4. Consume evidenced contracts, not historical title/status alone. Historical providers supply qualified component evidence; this contract provides the missing original-source assurance. Its unfinished deliverables are not entry prerequisites. Readiness separately audits external inputs.

- `PRE-028` — M19 original support acceptance; PRE-009 — retained M4 VFS/corpus component; PRE-008 — user read-only roots at retail initialization validation. PRE-029 is produced here.
...
Produces `PRE-029` (M20 original factory/data lifecycle); inspect this contract's source-owned positive and negative acceptance evidence before downstream consumption.

## Functional Requirements

Witness GameMain -> GameEngine::init/execute and source-owned initialization outputs. Missing data/factories and initialization-stage failures unwind exactly once in reverse order. Pending gameplay may be explicitly unavailable but never replaced by a toy loop.
...
Project-owned fixtures plus gated retail INI/CSF initialization; absent/malformed config, missing factory, all-stage failure/reverse teardown; arbitrary CWD and isolated XDG roots; assert no GPU/window acquisition in headless/verify modes.
...
Actual simulation and presentation remain later consumers. Required proprietary gaps follow the base-plan evidence gate. Component tests do not establish original execution. Unexpected required asset/backend limits follow base-plan §9 evidence-gated escalation, never reduced silent coverage.
